...
int main()
{
	...
	bool is_vessel_point(int i, int j);
	...
	for (i = 0; i <= P; i++)
		for (j = 0; j <= P; j++)
			for (k = 0; k <= Q; k++)
			{
				C1[i][j][k] = initial(x[k]);  
			}

	for (i = 0; i <= P; i++)
		for (j = 0; j <= P; j++)
		{
			U1[i][j] = is_vessel_point(i, j) * V_u;
		}
...
bool is_vessel_point(int i, int j) {
    int r1[] = {10, 60, 5, 90, 15, 30, 80, 20};
    int r2[] = {10, 5, 20, 30, 45, 35, 65, 80};
    int size = 8; 
